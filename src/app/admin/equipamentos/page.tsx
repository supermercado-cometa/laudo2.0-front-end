...
export default function EquipamentosPage() {
  ...
  const fetchEquipamentos = useCallback(async () => {
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/equipamentos?includeInactive=true`, {
  ...
  const filtered = equipamentos.filter(e => e.nome.toLowerCase().includes(searchTerm.toLowerCase()));

  return (
    <AdminPageLayout
      title={`Gestão de\nEquipamentos`}
      subtitle="Cadastre e organize os tipos de ativos e hardwares que serão auditados pelo time técnico."
      icon={Monitor}
      backUrl="/admin"
    >
      <div className="mb-12">
        ...
        <div className="relative mb-8 group">
          ...
          <input 
            type="text" 
            placeholder="Pesquisar equipamento (ex: balança, PDV)..." 
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center p-20 animate-pulse">
            ...
            <p className="mt-4 text-gray-400 text-[10px] uppercase tracking-widest text-center">Sincronizando inventário técnico...</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-4">
            ...
            {filtered.length === 0 && !isLoading && (
               <div className="p-20 text-center bg-gray-50/50 rounded-[32px] border-2 border-dashed border-gray-200">
                 <p className="text-[#9CA3AF] uppercase text-[12px] tracking-widest">Nenhum ativo identificado.</p>
               </div>
            )}
          </div>
        )}
           ...
           <div className="bg-white rounded-[32px] p-8 lg:p-12 max-w-lg w-full shadow-[0_32px_64px_-12px_rgba(0,0,0,0.2)] animate-in zoom-in duration-300">
             <div className="flex justify-between items-start mb-10">
                <div>
                  ...
                  <p className="text-gray-400 text-sm">Identifique o tipo de hardware para o sistema.</p>
                </div>
             ...
             <div className="space-y-8">
                 <div className="space-y-3">
                   <label className="text-[#9CA3AF] text-[10px] uppercase tracking-wider">Identificação do Equipamento</label>
                   <input 
                     type="text" 
                  ...
                  <div className="flex items-center gap-3 py-2">
                    ...
                    />
                    <label htmlFor="ativo" className="text-gray-700 font-medium cursor-pointer">Ativo Disponível</label>
                  </div>
                  <div className="pt-4">
                   <button 
                     onClick={handleSave} 
                   ...
                   >
                     <Save className="w-5 h-5" /> {editingId ? 'Salvar Edição' : 'Registrar Ativo'}
                   </button>
                 </div>
             </div>
          </div>
