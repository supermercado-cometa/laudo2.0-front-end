          ...
          <div className="grid grid-cols-1 gap-4">
              ...
              <div key={loja.id} className="w-full bg-white p-6 rounded-[24px] lg:rounded-[32px] flex items-center gap-6 shadow-sm group border border-transparent hover:border-[#003B99]/10 transition-all">
                ...
                <div className="flex gap-2 shrink-0">
                  <button 
                   ...
                   <button 
                    onClick={() => router.push(`/admin/lojas/${loja.id}`)}
                    className="w-10 h-10 rounded-xl bg-gray-50 text-gray-300 flex items-center justify-center hover:bg-white hover:text-[#1A4CAB] transition-all border border-transparent hover:border-blue-100"
                   >
            ...
            {filteredLojas.length === 0 && !isLoading && (
              <div className="p-20 text-center bg-gray-50/50 rounded-[32px] border-2 border-dashed border-gray-200">
                <p className="text-[#9CA3AF] font-bold uppercase text-[12px] tracking-widest">Nenhuma unidade corresponde aos filtros.</p>
              </div>
            )}
          </div>
        )}
           ...
           <div className="bg-white rounded-[32px] p-8 lg:p-12 max-w-lg w-full shadow-[0_32px_64px_-12px_rgba(0,0,0,0.2)] animate-in zoom-in duration-300">
             ...
             <div className="space-y-8">
                 <div className="space-y-3">
                   <label className="text-[#9CA3AF] text-[10px] font-black uppercase tracking-wider uppercase">Identificação (Nome)</label>
                   <input type="text" placeholder="Ex: Cometa Supermercados" value={formData.nome} onChange={e => setFormData({...formData, nome: e.target.value})} className="w-full h-12 rounded-xl bg-gray-50 border-none focus:bg-white focus:ring-2 focus:ring-[#1A4CAB] px-6 font-bold text-lg outline-none transition-all text-[#1A1A2E] placeholder:text-gray-300" />
                 </div>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                   <div className="space-y-3">
                     <label className="text-[#9CA3AF] text-[10px] font-black uppercase tracking-wider uppercase">Cód. Filial</label>
                     <input type="text" placeholder="Ex: 01" value={formData.filial} onChange={e => setFormData({...formData, filial: e.target.value})} className="w-full h-12 rounded-xl bg-gray-50 border-none focus:bg-white focus:ring-2 focus:ring-[#1A4CAB] px-6 font-bold text-lg outline-none transition-all text-[#1A1A2E] placeholder:text-gray-300" />
                   </div>
                   <div className="space-y-3">
                     <label className="text-[#9CA3AF] text-[10px] font-black uppercase tracking-wider uppercase">Cidade</label>
                     <input type="text" placeholder="Ex: Fortaleza" value={formData.cidade} onChange={e => setFormData({...formData, cidade: e.target.value})} className="w-full h-12 rounded-xl bg-gray-50 border-none focus:bg-white focus:ring-2 focus:ring-[#1A4CAB] px-6 font-bold text-lg outline-none transition-all text-[#1A1A2E] placeholder:text-gray-300" />
                   </div>
                 </div>
                 ...
                 <div className="pt-4">
                   ...
                   >
                     <Save className="w-5 h-5" /> {editingId ? 'Salvar Edição' : 'Cadastrar Unidade'}
                   </button>
                 </div>
             </div>
          </div>
