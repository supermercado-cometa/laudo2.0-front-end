...
import { Briefcase, Search, Loader2, Trash2, Edit3, Plus, X, Save } from "lucide-react";
...
export default function SetoresPage() {
  ...
  return (
    <AdminPageLayout
      title={`Gestão de\nSetores`}
      subtitle="Organize as áreas e departamentos das lojas que serão objeto de análise nos laudos técnicos."
      icon={Briefcase}
      backUrl="/admin"
    >
      <div className="mb-12">
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            {filtered.length === 0 && !isLoading && (
              <div className="p-20 text-center bg-gray-50/50 rounded-[40px] border-2 border-dashed border-gray-200">
                <p className="text-gray-400 font-bold uppercase text-[12px] tracking-widest">Nenhum setor identificado.</p>
              </div>
            )}
          </div>
        )}
          ...
          <div className="bg-white rounded-[40px] p-8 lg:p-12 max-w-lg w-full shadow-[0_32px_64px_-12px_rgba(0,0,0,0.2)] animate-in zoom-in duration-300">
             ...
             <div className="space-y-8">
                <div className="space-y-3">
                  <label className="text-[11px] font-black uppercase text-gray-400 tracking-wider">Identificação do Setor</label>
                  <input 
                    type="text" 
                    placeholder="Ex: Frente de Loja" 
                    value={formData.nome} 
                    onChange={e => setFormData({ nome: e.target.value })} 
                    className="w-full h-18 rounded-2xl bg-gray-50 border border-transparent focus:border-[#003B99] focus:bg-white px-6 font-bold text-lg outline-none transition-all" 
                  />
                </div>
                ...
                <div className="pt-4">
                  ...
                  >
                    <Save className="w-6 h-6" /> {editingId ? 'Salvar Edição' : 'Registrar Setor'}
                  </button>
                </div>
             </div>
          </div>
